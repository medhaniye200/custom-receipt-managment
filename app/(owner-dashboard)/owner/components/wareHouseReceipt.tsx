...
export default function ReceiptUpload() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate if withholding receipt is required but not provided
    ...
    // Only include withholding receipt if selected
    if (hasWithholding === "yes" && withholdingReceipt) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        try {
          ...
          setStatus(`Upload failed: ${errorData.message || errorData.error || "Please try again"}`);
        } catch {
              ...
              <p className="mt-2 text-sm text-gray-600">
                {mainReceipt ? (
                  <span className="font-medium text-blue-600">
                    {mainReceipt.name} ({(mainReceipt.size / 1024 / 1024).toFixed(2)}MB)
                  </span>
                ) : (
                  <>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  inputs.forEach(input => {
                    if (input.getAttribute('accept') === "application/pdf,image/*") {
                      (input as HTMLInputElement).value = "";
                    }
                  });
                }}
            ...
            <label className="flex flex-col items-center px-4 py-6 bg-white rounded-lg border-2 border-dashed border-blue-400 cursor-pointer hover:bg-blue-50 relative">
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="mt-2 text-sm text-gray-600">
                  {withholdingReceipt ? (
                    <span className="font-medium text-blue-600">
                      {withholdingReceipt.name} ({(withholdingReceipt.size / 1024 / 1024).toFixed(2)}MB)
                    </span>
                  ) : (
                    <>
                      <span className="font-medium">Click to upload</span> or drag and drop
