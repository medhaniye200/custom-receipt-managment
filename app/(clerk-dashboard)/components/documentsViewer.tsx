...
const createDataUrl = (base64String: string | null | undefined, label: string): string => {
  ...
  let mimeType = "";
  if (label.toLowerCase().includes("image") || label.toLowerCase().includes("custom file")) {
    mimeType = "image/jpeg"; // Common default for images
  } else if (label.toLowerCase().includes("permit") || label.toLowerCase().includes("invoice") || label.toLowerCase().includes("pdf")) {
    mimeType = "application/pdf";
  } else {
    // If we can't determine, use a generic binary type and log a warning
    console.warn(`Unknown document type for label: "${label}". Defaulting to "application/octet-stream".`);
...
function PreviewModal({ isOpen, onClose, fileUrl, fileLabel }: PreviewModalProps) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl h-full max-h-[90vh] flex flex-col">
        ...
        <div className="flex-grow p-4 overflow-auto">
          {isImage ? (
            <img src={fileUrl} alt={fileLabel} className="max-w-full max-h-full mx-auto object-contain" />
          ) : isPdf ? (
            <iframe
              src={fileUrl}
...
function FilePreview({ label, url, onPreviewClick }: { label: string; url: string; onPreviewClick: (url: string, label: string) => void }) {
  if (!url) {
    return (
  ...
  const handleDownload = () => {
    try {
      if (!url || !url.includes(',')) {
        console.error("Invalid data URL format for download (missing comma or empty URL):", url);
      ...
      if (!base64Content) {
          console.error("Base64 content is empty after splitting and cleaning:", url);
      ...
      a.download = `${label.replace(/[^a-zA-Z0-9]/g, '_')}.${mimeType.split('/')[1] || 'file'}`;
      ...
      alert(`Failed to download file: ${error instanceof Error ? error.message : "Unknown error"}. Please check console for details.`);
    }
  };
    ...
    <div className="bg-gray-100 p-4 rounded shadow flex flex-col justify-between">
      <div>
        ...
        {(isImage || isPdf) ? (
            ...
            >
                {isImage && <img src={url} alt={label} className="w-full h-full object-contain" />}
                {isPdf && <p className="text-blue-600 text-center p-4">Click to view PDF</p>}
            </div>
        ) : (
          <p className="text-red-500">Unsupported file format for preview. Cannot display.</p>
        )}
      </div>
...
export default function CustomFileViewer() {
    ...
    useEffect(() => {
      const fetchFiles = async () => {
        ...
        try {
          ...
          const response = await axios.get(`${BASE_URL}/api/v1/clerk/customfileAll`, {
          ...
          if (axios.isAxiosError(err) && err.response) {
              setError(`Failed to fetch documents: ${err.response.status} - ${err.response.data.message || err.response.statusText}`);
          } else {
    ...
    if (loading) {
      return <div className="text-center p-6 text-xl text-blue-600">Loading documents...</div>;
    }
  
    if (error) {
      return <div className="text-center p-6 text-red-600 border border-red-300 bg-red-50 rounded-md mx-auto max-w-md">Error: {error}</div>;
    }
  
    return (
      <div className="p-4 bg-gray-50 min-h-screen">
        <h2 className="text-3xl font-bold mb-8 text-center text-gray-800">User Document Viewer</h2>
        {userDocuments.length === 0 ? (
          <p className="text-center text-gray-600 text-lg py-10">No documents available for any user.</p>
        ) : (
          userDocuments.map((user, index) => (
            <div key={index} className="bg-white rounded-lg shadow-lg p-6 mb-10 border border-gray-100">
              <h3 className="text-2xl font-bold mb-4 text-purple-700 border-b pb-2">User: {user.firstname} {user.lastname}</h3>
              <p className="text-gray-700 mb-1"><strong>User ID:</strong> <span className="font-mono text-sm">{user.userId}</span></p>
              <p className="text-gray-700 mb-1"><strong>TIN Number:</strong> {user.tinNumebr}</p>
              <p className="text-gray-700 mb-4"><strong>Company:</strong> {user.companyname}</p>
              ...
              {user.documents.length > 0 ? (
                ...
                <p className="text-gray-600 italic mt-4 text-center py-4 border-t border-gray-200">No specific files uploaded for this user.</p>
              )}
            </div>
          ))
        )}
