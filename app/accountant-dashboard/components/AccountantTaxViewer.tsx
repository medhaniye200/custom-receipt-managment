...
export default function DeclarationList() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        openAlertDialog(
          "Fetch Error",
          err.message ||
            "An unexpected error occurred while fetching declarations."
        );
      } finally {
        setLoading(false);
      }
  ...
  const groupedDeclarations = declarations.reduce<GroupedDeclarations>(
    (acc, dec) => {
      ...
      <div className="font-sans max-w-6xl mx-auto p-8 bg-gray-100 min-h-screen">
        ...
        {declarationsForCompany.length === 0 ? (
          <p className="text-center text-gray-500 text-xl py-10">
            No declarations found for this company.
          </p>
        ) : (
            ...
            {declarationsForCompany.map((dec, index) => (
              ...
              >
                <h3 className="text-xl font-bold mb-4 text-blue-700">
                  Declaration #{dec.declarationNumber}
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-4 text-gray-700 text-base">
                  <p>
                    <strong className="text-gray-900">TIN Number:</strong>{" "}
                    {dec.iteminfo[0]?.tinnumber}
                  </p>

                  <p>
                    <strong className="text-gray-900">HS Code:</strong>{" "}
                    {dec.iteminfo[0]?.hscode}
                  </p>
                  <p>
                    <strong className="text-gray-900">Item:</strong>{" "}
                    {dec.iteminfo[0]?.itemdescription}
                  </p>
                  <p>
                    <strong className="text-gray-900">Quantity:</strong>{" "}
                    {dec.iteminfo[0]?.quantity}
                  </p>
                  <p>
                    <strong className="text-gray-900">Unit Price:</strong>{" "}
                    {dec.iteminfo[0]?.unitprice.toLocaleString()}
                  </p>
                  <p>
                    <strong className="text-gray-900">DPV Amount:</strong>{" "}
                    {dec.dpvAmountPerDeclaration.toLocaleString()}
                  </p>
                  <p>
                    <strong className="text-gray-900">Duty Tax:</strong>{" "}
                    {dec.dutyTax.toLocaleString()}
                  </p>
                  <p>
                    <strong className="text-gray-900">Excise Tax:</strong>{" "}
                    {dec.exciseTax.toLocaleString()}
                  </p>
                  <p>
                    <strong className="text-gray-900">Scanning Fee:</strong>{" "}
                    {dec.scanningFee.toLocaleString()}
                  </p>
                  <p>
                    <strong className="text-gray-900">Scanning Fee VAT:</strong>{" "}
                    {dec.scanningFeeVAT.toLocaleString()}
                  </p>
                  <p>
                    <strong className="text-gray-900">
                      Social Welfare Tax:
                    </strong>{" "}
                    {dec.socialWelfareTax.toLocaleString()}
                  </p>
                  <p>
                    <strong className="text-gray-900">Surtax:</strong>{" "}
                    {dec.surtax.toLocaleString()}
                  </p>
                  <p>
                    <strong className="text-gray-900">VAT:</strong>{" "}
                    {dec.vat.toLocaleString()}
                  </p>
                  <p>
                    <strong className="text-gray-900">Withholding Tax:</strong>{" "}
                    {dec.withholdingTax.toLocaleString()}
                  </p>
  ...
  return (
    <div className="font-sans max-w-4xl mx-auto p-8 bg-gray-100 min-h-screen">
      ...
      {Object.entries(groupedDeclarations).length === 0 ? (
        <p className="text-center text-gray-500 text-xl py-10">
          No companies with declarations available.
        </p>
      ) : (
        <div className="space-y-6">
          {Object.entries(groupedDeclarations).map(
            ([companyName, declarationsForCompany]) => (
              <div
                key={companyName}
              ...
              >
                <div>
                  ...
                  <p className="text-gray-600 text-sm">
                    {declarationsForCompany.length} declaration(s)
                  </p>
                </div>
                <button
